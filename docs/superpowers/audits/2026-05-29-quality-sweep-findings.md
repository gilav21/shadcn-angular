...
| Morning-review follow-up (carousel RTL init + chat avatar mirror) | 2026-05-29 | 96 | **New findings, user-reported during PR #70 review.** (1) **carousel — CRITICAL RTL:** nav buttons use physical `left-2`/`right-2` and depend entirely on `rtl()`; on a *direct* rtl load (navigated to while `dir=rtl`, not toggled) `rtl` stayed false (constructor `getComputedStyle` race + change-only `MutationObserver`) → wrong button active, clicking it a no-op. Fix: re-read RTL in the post-content-init timer. (2) **chat — MAJOR layout/RTL:** user-message avatar floated inboard near center instead of mirroring the assistant's outer-edge avatar. Fix: `[&>ui-avatar]:order-last` on the user row (writing-mode-safe, LTR+RTL). Live browser repro + fix verified (RTL carousel advances 1→2; ME avatar moved 451→711px to outer edge); fix-sensitive unit tests added; 45 tests pass; demo build clean. |
