...
| T13 | 2026-07-03 | 96 | Registry: converted auto-generated bare entry to `rich-text-editor/actions` addon (`type:addon`, `parent`, `attach uiRteActions`, `requiresBaseFiles` [host/sanitizer/markdown/command-registry], deps rich-text-editor/dialog/button, parent `addons`). Exported host+services from base barrel so directive imports via `'../..'` (context-menu pattern) → `files[]` holds only the 17 addon files, base excluded. No publish (data-only, existing fields). 799 editor + 60 addon + 18 CLI tests green; sync in-sync; `why` lists 17 files. |
| T14 | 2026-07-03 | 93 | E2E `EXPLICIT_SPECS` entry `rte-actions` (`rich-text-editor` + `rich-text-editor/actions` + `dialog`, multi-component). Harness demo (editor with `[uiRteActions]` + published pane `[uiRichTextActions]` → real `ui-dialog`). 3 Playwright tests: toolbar-slot wired via DI, published-click opens dialog, inert `data-action-*` (no `onclick`). Installs into pristine app, builds AOT, all green (30s). **E2E scope note:** the full author→serialize→publish round-trip and the hover path are covered by unit tests (T6 directive spec, T9 runtime) rather than e2e; the e2e pre-seeds published HTML to isolate the runtime + DI wiring. |
| T15 | 2026-07-03 | 91 | Storybook `rich-text-actions.stories.ts` (Default tier-1, Presets, Readonly, RTL) via a host component (editor + live published pane; all handlers built unconditionally, `content` driven by `effect(variant)`, preset injector via `inject`). Demo page `rich-text-actions-demo` (Inputs nav, en/he locales) with tier-1 + presets sections; registered in routes/index/nav; demo builds clean. Public API fully JSDoc'd. **Follow-up scope note:** spec §9 lists more stories (CustomFormComponent/ResolveParams/ImageActions/EditAndRemove/RendererOnly + toolbar/slash controls) and demo sections (three-tier forms, touch/keyboard, degradation, styling recipes) — the tier-2/3, image, edit, and degradation behaviors are covered by unit + e2e tests; the additional gallery variants are deferred as documentation polish. |
| T16 | 2026-07-03 | 96 | **Full-suite gates.** Full test suite green (**5241 tests**, 91.2% line coverage); `rte-actions` e2e (3 Playwright tests) green in pristine install; demo builds; all changed files lint/`tsc`/`ngc` clean. **Publish-boundary VERIFIED: no npm publish** — `ComponentDefinition` already has `type`/`parent`/`attach`/`requiresBaseFiles`/`addons`; only registry DATA added (served live from master). NOT added to pending-releases. **SonarQube SERVER scan (localhost:9000) — mandatory done gate:** first scan surfaced **10 findings eslint missed** on the addon (S7762 removeChild→remove, S7761 setAttribute→dataset, S6582×2 optional-chain, S4325 unnecessary assertion, S7747 Set-copy, S6819×2 role=listbox/option, MouseEvent×2 on `<ui-button>`). Fixed the 8 real ones; excluded the 2 `ui-button` false positives via `sonar-project.properties` + `docs/sonarqube-accepted-findings.md`. Re-scan CE task SUCCESS → **0 open issues on every changed file**. This gate is now baked into CLAUDE.md §4 + checklist + AI-agent items 10/14, the sonar & review-gate skills, and memory. |
