...
| T16 | 2026-07-03 | 96 | **Full-suite gates.** 1329 unit tests green across all touched areas (rich-text-editor 799 incl. addon 60, data-table 530, lib). `rte-actions` e2e (3 Playwright tests) green in pristine install; demo builds. All changed files lint clean + `tsc`/`ngc` clean (the 4324 `check:all` errors were entirely pre-existing `.claude/worktrees/` noise, not this branch). Fixed 3 sonar nits in touched spec/e2e files (toHaveLength, `\w`, bypass disable). e2e impact decision = ALL (shared lib touched). **Publish-boundary VERIFIED: no npm publish** — `ComponentDefinition` already has `type`/`parent`/`attach`/`requiresBaseFiles`/`addons` (context-menu), `isValidRegistryEntry` already handles `type:'addon'`; only registry DATA added (snapshot regenerated by sync, served live from master). NOT added to pending-releases. |
