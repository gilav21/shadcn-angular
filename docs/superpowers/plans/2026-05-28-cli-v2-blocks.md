...
| T19 blocks sync hook | 2026-05-28 | 97 | Registry tooling made block-aware: `validate-registry.mjs` adds `packages/blocks/` to its relevance check (block edits re-run sync); `sync-registry.ts` `detectOrphanBlockFolders` warns (non-fatal, stdout) about any `packages/blocks/<name>/` lacking a registry entry, surfaced via the hook's `additionalContext`. Auto-append stays UI-only (uiRelativePath null trace). E2E-verified; 32 registry tests pass. NOTE: these are dev tooling (`.claude/`, `scripts/`) — not part of the published package; block `dependencies` stay hand-maintained (Spec B). |
| T20 block drift detection | 2026-05-28 | 96 | Closed T19's deferred gap. `walkBlockTree` (sync-registry-lib) import-walks blocks across packages/blocks → packages/components, deriving files/dependencies/libFiles; `analyzeBlock` puts blocks through the same drift→report(exit 1)/`--fix`(auto-correct) flow as components while never touching hand-authored type/category/description/tags. Own-folder guard prevents sibling-block absorption; closures mirror walkTree to stay <CC15. Verified: no false positives, drift caught + auto-fixed (deps & libFiles), meta preserved; 4 block deps canonicalized. 196 tests (4 new), build clean. Behavior change: a declared-but-deleted block file is now auto-healed (drift) rather than a hard error — matches components. Supersedes T8. |
...
| — integration | 2026-05-28 | verified | All 6 registered (sync `--fix` derived deps == hand-authored, in sync); registry-meta pass; demo `ng build` clean; fixture install (signup forgot-password settings-account hero features faq) + ng build clean; all 6 demo pages visually QA'd at :4200. |
