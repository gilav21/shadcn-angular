...
| 6 — Migration robustness + addon↔base compat | 2026-06-29 | 96 | **Capability-based compat (not version/hash — edit-robust):** addons declare `requiresBaseFiles` (the base files embodying the contract, e.g. `data-table/data-table.host.ts`); `apply` checks they exist in the installed base after install-if-missing and refuses with "run `update <base>` first" when absent — so an *edited* base still wires (only the contract's presence matters). `missingBaseFiles` pure+tested. **Breaking-change surfacing in the CLI:** `printBreakingChanges` wired into `update` (conflicting+toInstall) and `doctor` (updateAvailable) — the data existed (`collectBreakingChanges`) but only MCP surfaced it. Regression test locks data-table's rowActions/enableColumnMenu breaking entries point at the migration. Verified end-to-end: `update data-table` prints the breaking notes; `apply` on a base missing `data-table.host.ts` (edited→declined) refuses and doesn't wire. CLI 458 pass; tsc+eslint+sync clean. |
...
| 4 — Resolver + add command: addon install | 2026-06-29 | 95 | `add` installs opt-in addons with lean-by-default precedence (`--no-addons` > `--with <parent/addon>` full-key > `--yes`/`--all` > interactive multiselect); the `add <parent>/<addon>` shorthand installs a single addon onto an already-installed base; addons are hidden from the component picker; a post-install discoverability hint lists a base's available addons on success/dry-run/nothing-to-install. (Recorded from commit `7369221`; `--with` full-key constraint hardened in `28bcd9c` after the `ai`-collision correction.) |
| 5 — apply command: AST wire + two-level targeting | 2026-06-29 | 95 | `apply` is the only command that edits app code: it installs the addon (+ base) if missing, then wires it via two-level targeting — file selection (component class name / project scan, app-code-only) and instance selection (`--all`/`--class`/`--id`), with a paste-snippet fallback when wiring is ambiguous. Idempotent, git-style. All source-editing helpers in `apply-wire.ts` are pure and ReDoS-safe (indexOf/per-char scans, no adjacent-star regex). (Recorded from commit `cc8fa03`.) |
| 10 — Migration robustness + addon↔base compat | 2026-06-29 | 96 | Capability-based compat guard: `apply` refuses to wire when the installed base is missing the addon's `requiresBaseFiles` contract file(s) (edit-robust — presence, not version/hash), pointing the dev at `update <parent>`. `printBreakingChanges` wired into `update` (conflicting+toInstall) and `doctor` (the primary upgrade path). A sync-registry regression test proves a consumer importing an addon barrel is auto-detected as a discovered dependency. (Recorded from commit `216181b`.) |
| 6 — MCP surfacing (`get_component` + `apply_addon`) + addon e2e gate | 2026-06-29 | 96 | `get_component` now surfaces a base's `addons[]` and an addon entry's `attach`/`parent`/`requiresBaseFiles`. Extracted a silent, non-exiting engine `core/apply-core.ts` (`applyCore`/`wireAddonCore`/`resolveAddonInfo`/`ApplyError`) — no stdout, no `process.exit`, no prompts (stdio-safe) — and refactored the interactive `commands/apply.ts` to consume its shared helpers (one-directional command→core→apply-wire, `ApplyOptions` relocated to break the cycle, no Task-5 regression). New MCP write tool `apply_addon` installs+wires non-interactively, validating the addon name before the init guard. New e2e harness `data-table-context-menu` installs the lean base + the addon and Playwright-tests the DI auto-wiring (⋮ button + right-click menu), passing 3/3. CLI 469 pass; MCP 11; apply-core 6; tsc+eslint clean. Reviewer nits (minor install-block duplication, double addon-resolve) accepted as harmless. |
| 11 — `update`: prompt to auto-`apply` an addon fix for a breaking change | 2026-07-01 | 94 | Real-world test against a customer project (`oneFile-1`) showed `update` warns about data-table's breaking change but leaves the consumer's build broken until the addon is applied by hand. Added `BreakingChange.suggestedAddon?: string` (data-table's two entries) + pure `collectSuggestedAddons` helper; `update.ts` now offers to run `apply <addon>` right after the base merge, skipped entirely under `--yes`. Deliberately reuses the interactive `apply()` (not `applyCore()`) so the user keeps its per-usage target-selection prompt. Iter 1 (83) flagged missing `--yes`-skip e2e coverage and an undocumented reuse choice; both fixed (new `addonSuggestionSkippedUnderYes` e2e scenario + code comment). Full detail in `2026-06-29-merge-update-design.md` Completion Log row 9 (this change lives mostly in `update.ts`, the merge-update command). |
