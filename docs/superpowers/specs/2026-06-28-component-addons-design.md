...
| 4 — Resolver + `add`: install addons | 2026-06-29 | 95 | `promptAddons` (mirrors `promptOptionalDependencies`) collects each resolved base's `addons[]` and is lean-by-default: precedence `--no-addons` > `--with <list\|all>` > `--yes` (none) > `--all` (all) > interactive (none pre-selected). `--with` matches short name or full key with exact-first + ambiguous-short warning; unknown tokens warn. Selected addons thread through the existing `optionalDeps` install pipeline (files/npm/lib install). `selectableComponentNames` hides addons from the picker/`--all` without touching shared `getComponentNames` (so `update`/`list` still see them); explicit `add parent/addon` works. Resolver invariants proven (addon→base+context-menu; base↛addon). `add` stays install-only. Verified end-to-end: real CLI install of `data-table/context-menu` + `ng build` passed. CLI 419+2 pass; tsc+eslint clean. |
