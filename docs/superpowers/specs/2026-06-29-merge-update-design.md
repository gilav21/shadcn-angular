...
| 8 | Full-review hardening — merge3 EOL normalization, MCP `apply_addon` mergeReport, **2nd-update data-loss fix** | 2026-06-30 | (full review) | A full end-to-end review (iter 1: 93, iter 2: 63) found: (a) merge3 must normalize CRLF→LF before diffing (else a Windows consumer copy spuriously whole-file-conflicts); (b) MCP `applyCore` discarded the install `mergeReport` (conflicts invisible to agents); (c) **shippable-blocking** — `mergeWriteFile` recorded the merged content as the manifest baseline, so a 2nd update fast-path-overwrote the user's earlier merged edits. Fix (c): record THEIRS (pristine upstream at the advanced ref) as the baseline. Guarded by a two-cycle unit test + a second-update e2e scenario (verified load-bearing). |
| 9 | `update`: prompt to auto-`apply` an addon fix for a breaking change | 2026-07-01 | 94 | Real-world validation against a customer project (`oneFile-1`) surfaced a gap: `update` warned about data-table's `rowActions`/`showRowActionsColumn`/`enableColumnMenu` breaking change but never offered the fix, so the merged base still failed `ng build` until the addon was applied by hand. Added `BreakingChange.suggestedAddon?: string` (additive, populated on data-table's two entries + `registry.json` regenerated in sync) and a pure `collectSuggestedAddons` dedup/sort helper (`core/plan.ts`, unit-tested). `update.ts`'s new `promptApplyAddons` runs after the base merge completes, skips entirely under `--yes` (no prompt, no auto-install — installing an addon edits app-code templates, a bigger action than the merge itself), and deliberately reuses the *interactive* `commands/apply.ts` `apply()` (not the silent `applyCore()`) so the user keeps its per-usage target-selection prompt after opting in. Iter 1 (83) flagged missing e2e coverage for the `--yes`-skip path and an undocumented `apply()`-vs-`applyCore()` choice; both fixed — `addonSuggestionSkippedUnderYes` e2e scenario (real upstream data-table change, asserts the addon is NOT auto-installed under `--yes`) and a code comment explaining the reuse is safe (`options.yes` is always false on this call path, so `apply()`'s internal exit guard can't fire). CLI 546 pass; `merge-update`/`data-table-context-menu` e2e 1/1 each; tsc+eslint clean. |
| 10 | v0.0.46 post-ship review remediation (branch `fix/v0046-review`) | 2026-07-02 | 93–97 per group | A 6-agent post-ship review (see `project_v0046-review-findings` memory; report delivered 2026-07-02) found 1 Critical + 6 High + 11 Medium findings across the addon+merge feature set. Fixed in 7 parallel worktree groups (each TDD + own review gate ≥93) + an inline wave 2: **Critical** unscoped `npx shadcn-angular` in the flagship migration note resolves to a FOREIGN npm package (fixed everywhere + regression spec `registry/breaking-notes.spec.ts`). **Highs:** piped non-TTY `update` auto-accepted the addon prompt (auto-installed!) → `shouldOfferAddonApply` gate skips when `!isTTY` (fixes review-9's own H1 defect); `apply` hardcoded the `ui-` tag → prefix-aware (`--prefix acme` installs now wire); `apply --yes` silently skipped multi-instance templates → wires all (`--all` semantics); fresh `init` failed `doctor` exit 1 → utils template synced + drift-guard spec (template≡lib via doctor's own `normalizeContent`) + init records lib baselines; curated `help`/README had zero mention of `apply`/`update`/addons/merge → documented; addon hint now says `apply` not `add`. **Mediums:** merge3 EOF-no-trailing-newline spurious conflict fixed (newline-terminate → merge → restore OURS EOF), merged files keep OURS's dominant EOL (CRLF no longer rewritten) + BOM + lone-`\r`; addon prompt gated on already-installed; `--overwrite` prints honest whole-file advice (no more "will 3-way merge"); ref-less components warned as keep-and-warn not promised a merge; merge summary split Added/Refreshed/Overwrote + lists merged filenames + "(files)" unit label; MCP `apply_addon`/`update_component` return `hadConflicts` + document `fellBack`; v1-lockfile read warns once; stray `--registry` no longer disables local-mode merging; transport vs 404 fetch errors distinguished; pristine 'untracked'/'modified' lib files matching a published baseline refresh (the 'modified' case found live by the `doctor-lib-drift` e2e after the init fix). **Wave 2 (inline, gate 94):** `breaking-scan.ts` — update/doctor print the consumer's own `file:line` usages of removed inputs (or a reassurance); `update --dry-run` predicts per-file outcomes via `decideMergeFile`/`previewMergeFile`/`previewComponentMerges` (byte-faithful refactor, verified); `components.json {update:{overwrite:true}}` team default. Deferred/accepted: per-file refs (design), dry-run preview omits peerFiles (nit), interrupted-update manifest staleness (verified self-healing), `apply --yes` wiring plain tables (by design — user invoked apply). Final: CLI 606/606, e2e 69/69, tsc+eslint clean. |
