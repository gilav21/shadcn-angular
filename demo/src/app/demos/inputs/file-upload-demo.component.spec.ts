import { TestBed } from '@angular/core/testing';
import { provideZonelessChangeDetection } from '@angular/core';
import { provideUiLocale } from '../../../../../packages/components/lib/i18n';
import { FileUploadDemoComponent } from './file-upload-demo.component';
import { FILE_UPLOAD_DEMO_LOCALES } from './file-upload-demo.locales';

describe('FileUploadDemoComponent', () => {
  it('renders English title under default locale', () => {
    TestBed.configureTestingModule({ providers: [provideZonelessChangeDetection()] });
    const fixture = TestBed.createComponent(FileUploadDemoComponent);
    fixture.detectChanges();
    expect(fixture.nativeElement.textContent).toContain(FILE_UPLOAD_DEMO_LOCALES['en'].title);
  });

  it('renders Hebrew title under provideUiLocale("he")', () => {
    TestBed.configureTestingModule({
      providers: [provideZonelessChangeDetection(), provideUiLocale('he')],
    });
    const fixture = TestBed.createComponent(FileUploadDemoComponent);
    fixture.detectChanges();
    expect(fixture.nativeElement.textContent).toContain(FILE_UPLOAD_DEMO_LOCALES['he'].title);
  });
});
