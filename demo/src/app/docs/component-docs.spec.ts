...
describe('T-8: StackBlitz link is well-formed for every demo page', () => {
    it('points at this repo and encodes the demo file', async () => {
        const payload = await loadPayload();
        const linked = payload.components.filter(c => c.stackblitz !== null);
        expect(linked.length).toBeGreaterThan(100);

        for (const component of linked) {
            const url = new URL(component.stackblitz as string);
            expect(url.origin).toBe('https://stackblitz.com');
            expect(url.pathname)
                .toBe('/github/gilav21/shadcn-angular/tree/master');
            expect(url.searchParams.get('file')).toBe(component.demoFile);
        }
    });

    it('gives a link exactly to the components that have a demo file', async () => {
        const payload = await loadPayload();
        for (const component of payload.components) {
            expect(component.stackblitz === null)
                .toBe(component.demoFile === null);
        }
    });

    it('renders the link on the demo page header', async () => {
        configure();
        await TestBed.inject(ComponentDocsService).load();
        const fixture = TestBed.createComponent(DocsHeaderComponent);
        fixture.componentRef.setInput('route', 'buttons');
        ...
        const link = (fixture.nativeElement as HTMLElement)
            .querySelector('[data-slot="stackblitz-link"]');
        expect(link?.getAttribute('href')).toContain('stackblitz.com/github/gilav21');
