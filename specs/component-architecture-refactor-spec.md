...
- [ ] **1.1** `sync-registry.ts` — `resolveImport` must resolve **directory /
  ...
  entry.
- [ ] **1.2** `sync-registry.ts` — the walker must discover **`templateUrl`,
  ...
  never be bundled.
- [ ] **1.3** `sync-registry.ts` — confirm component-boundary detection
...
- [ ] **1.4** `validate-registry.mjs` — replace the name-based new-component
...
- [ ] **1.5** `validate-registry.mjs` — keep the sync-first ordering and the
