...
| 3 | 2026-07-09 | T-B1 — combined-action types (`ActionParamsMode`, `combined?`, `paramsMode?`, `fieldsByTrigger?`) + `writeCombined`/`isCombinedOnElement` in the serializer | 97/100 | Diff matches the task-3 brief and §14.3 verbatim; `isCombinedOnElement`'s `click !== null && click === hover` guard is correct across all four edge cases (only-click, only-hover, both-different, both-same), traced against `writeAction`'s attribute semantics. 13/13 serializer tests green, `tsc --noEmit` clean on touched files, strictly types-and-serializer scope (no UI/picker code). SonarQube server scan (§4 DONE gate) could not be run in this environment — Docker unavailable and no `SONAR_TOKEN` in the shell env — flagged as outstanding, not silently skipped. |
| 4 | 2026-07-09 | T-B2 — combined-action dialog (single picker row, no trigger radio; shared/separate params forms; directive attaches both triggers + starter seed in one transaction) | 96/100 | `resolveParamsMode()` correctly falls back `combined+separate` to `shared` when a `formComponent`/`resolveParams` tier is present (unsupported per §14.3), replacing round-1's gap where the broken two-group UI stayed reachable with an always-valid empty form; the directive's new `applyResolvedParams()` routes tier-3 `resolveParams` through `applyCombined()` for combined defs so `shared`-mode resolveParams also writes identical params to both triggers in one `mutateContent`/`wrapSelection` transaction (one undo entry). 40/40 tests green across both spec files, zero eslint findings, complexity kept low via extracted helpers (`resolveParamsMode`, `confirmCombined`, `applyResolvedParams`, `warnOnCombinedMisuse`). Round 1 scored 83/100 (missing shared-mode fallback + zero test coverage on the guard); round 2's fix closed both gaps, with only a minor noted nit (directive-level misuse test exercises the `resolveParams` half of the OR-guard directly, `formComponent` half covered indirectly via the dialog test). SonarQube server scan (§4 DONE gate) could not be run in this environment — Docker unavailable and no `SONAR_TOKEN` — flagged as outstanding, not silently skipped. |
| 5 | 2026-07-09 | T-B3 — edit popover collapses a combined action to a single row and removes both triggers | 96/100 | `buildPopoverRows` (`rich-text-actions.directive.ts:287-297`) collapses a combined element into one `PopoverActionRow` with `combined: true` via `isCombinedOnElement`, and `removeCombined` (`rich-text-actions.directive.ts:364-380`) clears both `data-action-click`/`data-action-hover` attributes, strips an unedited seed, unwraps a bare span, and emits `actionRemoved` once per trigger — mirroring `removeTrigger`'s existing single-trigger logic. Two separate single-trigger actions on one span still render two rows (regression test passes), and the Task 4 follow-up fix now reads `confirmLabel` as "Replace" for a combined action when either trigger is occupied (`rich-text-actions-dialog.component.ts:129-139`), not just click. 46/46 tests green across the three touched spec files, zero eslint findings, zero new `tsc` errors; only a documented minor nit (the combined row's trigger label hardcodes click/hover order rather than iterating `def.triggers`, a defensible simplification with only two triggers today). SonarQube server scan (§4 DONE gate) could not be run in this environment — Docker unavailable and no `SONAR_TOKEN` — flagged as outstanding, not silently skipped. |
| 6 | 2026-07-09 | T-C1 — `linked-preview-dialog` combined preset (hover shows a preview card, click opens a dialog) | 95/100 | `linkedPreviewDialogAction`/`linkedPreviewDialogHandlers` (`presets/linked-preview-dialog.preset.ts`) return `combined:true, triggers:['click','hover']` and a single `{[id]:handler}` map that branches on `event.trigger`, delegating (round-2 fix, not the round-1 hand-rolled version) to `hoverCardHandlers`/`openDialogHandlers` re-keyed onto the combined id — genuinely reusing their grace-area/Esc-close and Esc-to-dismiss logic instead of reimplementing overlay lifecycle, with `hover`/`dialog` forwarding options now wired through and a `confirmLabel` field added to match spec §14.4's described field set. Round 1 scored 82/100 (grace-area/Esc logic dropped, forwarding options dead); round 2 scored 94/100 (delegation fixed, one nit on the missing `confirmLabel` field); round 3's fix closed that gap. 23/23 tests green across the four presets spec files, zero eslint findings. SonarQube server scan (§4 DONE gate) could not be run in this environment — Docker unavailable and no `SONAR_TOKEN` — flagged as outstanding, not silently skipped. |
| 7 | 2026-07-09 | T-E1 — swap `fontColor`/`backgroundColor` swatch grids for `ui-color-picker`, keeping `colorPalette`/`highlightPalette` as `presets` (`alpha` on the background picker only), wiring `colorChange` → the unchanged `onColorSelect(type, color)` | 96/100 | `rich-text-toolbar.component.html` binds `[presets]="colorPalette"` (text, no alpha) and `[presets]="highlightPalette" [alpha]="true"` (background), both via `(colorChange)="onColorSelect(...)"`, leaving `onColorSelect`'s signature and the `colorSelect` output contract untouched; two new `currentFontColor`/`currentBackgroundColor` inputs default to `''`, deferred to T-E2. Round 1 scored 74/100 for two claims: an `NG0955` duplicate-key console warning on `highlightPalette`, and tests that only checked picker-existence/a direct `onColorSelect()` call rather than a real DOM click through the picker's presets. Investigation (commit `248445a6`) confirmed the `NG0955` warning is pre-existing (same `track color` expression already present in the removed raw swatch grid on the parent commit) and added two DOM-driven preset-click tests, including the `'transparent'` background preset, which locks in the verified current-behavior emission (`#00000000` — `parseColor('transparent')`→`{a:0}`→`formatHex(..., alpha=true)`), documented as CSS-equivalent to the literal string rather than an unverified assumption. Round 2 scored 96/100: wiring, contract, and test-DOM verification all independently reproduced; SonarQube server scan (§4 DONE gate) could not be run in this environment — Docker unavailable and no `SONAR_TOKEN` — flagged as outstanding, not silently skipped. |
| 8 | 2026-07-09 | T-E2 — `detectCurrentColors()` reflects the selection's computed color/background-color into `currentFontColor`/`currentBackgroundColor`, reusing `lib/color`'s `parseColor`/`formatHex` and a shared `selectedElement()` helper extracted from `detectCurrentFontSize`/`detectCurrentFontFamily`, bound into both the top and floating `ui-rich-text-toolbar` instances | 97/100 | `detectCurrentColors()` (`rich-text-editor.component.ts:5619-5631`) calls `updateActiveFormats()`'s new third detector, normalizing `getComputedStyle().color`/`.backgroundColor` via `toHexColor()`/`isTransparentColor()` which delegate to `parseColor`/`formatHex` from `../../lib/color` rather than duplicating rgb↔hex math; `isTransparentColor` returns `!rgba || rgba.a === 0` so `rgba(...,0)` and the literal string `'transparent'` (resolved by `parseColor`'s `NAMED_COLORS` table to `a:0`) both yield an empty active swatch. Round 1 scored 94/100 for one nit: a dead `|| value === 'transparent'` clause in `isTransparentColor` that could never be reached since `parseColor('transparent')` always short-circuits the preceding `rgba.a === 0` check; the clause was removed and round 2 independently re-verified the fix changed no behavior. Three new `describe('current colors', ...)` tests drive real DOM selection (`selectAllOf`) over styled/unstyled spans rather than fabricated state; full spec file 294/294 green, `tsc --noEmit` clean. SonarQube server scan (§4 DONE gate) could not be run in this environment — Docker unavailable and no `SONAR_TOKEN` — flagged as outstanding, not silently skipped. |
| 9 | 2026-07-09 | T-E3 — registry: confirm `color-picker` is registered as a `rich-text-editor` dependency; regression + RTL/touch/density pass; publish-boundary re-verification | 94/100 | `sync-registry:fix` produced a zero-diff, zero-warning run — `color-picker` was already present in `rich-text-editor`'s `dependencies` (added by the T-E1 commit `75ea5a66`, confirmed via `git log`/`git show`), so no new registry change or commit was needed for this task. `validate-registry.mjs` exits 0. Both named regression specs pass in full: `rich-text-toolbar.component.spec.ts` 54/54, `rich-text-editor.component.spec.ts` 294/294, including all 7 RTL-named `it()` tests across the two files (3 toolbar, 4 editor `Locale and RTL` block) and the transparent-preset color test; touch coverage for the picker itself lives in `ui-color-picker`'s own untouched spec (unaffected by this wiring-only change), and no new layout CSS means no density regression is possible. `load.ts:38-50`'s `isValidRegistryEntry`/`isValidRegistryShape` only checks `name`/`files`/`addons`/addon-attach shape, never `dependencies`, and `packages/cli/src/**` has zero diff from this task — confirming **no publish required**. Round 1 scored 78/100 for not explicitly addressing the plan Task 9 title's "RTL/touch/density" phrase in the original summary (the step-by-step task body has no separate step for it; that deeper work is Task 10/T-D1's scope); round 2 scored 84/100 after the report was expanded, docking for an inflated RTL-test-count figure (a raw `grep -c "rtl"` line count, not an `it()`-title count) and this Completion Log row not yet existing at review time; round 3 (final) scored **94/100** confirming both fixes independently (row present and well-formed, RTL count verified exactly 7 — 4 at `rich-text-editor.component.spec.ts:1006,1014,1078,1089` + 3 at `rich-text-toolbar.component.spec.ts:133,148,158`), registry/`load.ts` shape-validator/publish-boundary claims re-confirmed unchanged, with the sole residual note being the still-unexercised SonarQube server scan — disclosed, not silently skipped, and moot here since this task changed zero source files (Docker/`SONAR_TOKEN` unavailable in this environment, consistent with every prior task in this plan). |
| 10 | 2026-07-09 | T-D1 — Storybook `StarterStyling`/`CombinedAction` stories + demo "6 · Starter styling + combined action" section (en/he) | 96/100 | `StarterStyling` binds `[uiRteActionsStyle]="starterStyle()"` to a non-default `STARTER_STYLE` object and `CombinedAction` wires `linkedPreviewDialogAction()`/`linkedPreviewDialogHandlers()` with a genuine both-trigger span (`data-action-hover`/`data-action-click` + matching `-params` attrs on one element, same preset id) matching the exact attribute shape the runtime produces for `combined:true`, per `rich-text-actions.directive.spec.ts:247-266`. The demo's new section 6 (`rich-text-actions-demo.component.html`) mirrors the existing sections' editor/published-pane grid, `SafeHtml` trust helper, badges, and copy-paste code block with a copy button, with symmetric en/he locale keys added to `rich-text-actions-demo.locales.ts`. `npm run build:demo`, `npx eslint` on the four changed files, and `npx tsc --noEmit -p .storybook/tsconfig.json` were all independently re-run clean by the reviewer; `git diff --name-only` confirmed no non-`.stories.ts` production file under `packages/components/ui/rich-text-editor/addons/actions/` was touched, satisfying the docs/demo-only constraint. Sole nit: `V2_SNIPPET`'s `tier: 'v2'` is loosely `string`-typed rather than a union — non-blocking. SonarQube server scan (§4 DONE gate) could not be run in this environment — Docker unavailable and no `SONAR_TOKEN` — flagged as outstanding, not silently skipped, and moot here since this task touched zero production files. |
| 11 | 2026-07-09 | T-D2 — extend the `rte-actions` e2e harness for combined-action + seeded-style (tests 84–85); add `color-picker` to the install set | 96/100 | `e2e/orchestrator/specs.ts`'s `rte-actions` entry gained `color-picker` (correct and minimal — `rich-text-toolbar.component.ts` now imports `ColorPickerComponent` per T-E1); the harness demo (`rte-actions-demo.component.ts`) added a second "combined action" editor + published pane wired to `linkedPreviewDialogAction()`/`linkedPreviewDialogHandlers()` and a `uiRteActionsStyle` seed, exposing the editor's raw emitted HTML via a `<pre>` for round-trip assertions. `rte-actions.spec.ts`'s `attachCombinedAction()` helper drives the real toolbar attach flow; test 84 splits into two isolated assertions (hover shows a real `[data-slot="preset-hover-card"]` with authored text, click opens a real `[role="dialog"]` with the authored title) rather than chaining hover-then-click in one test, avoiding Playwright's own mouse-move-before-click re-triggering the hover preview; test 85 checks both the emitted `style="` attribute in the editor's HTML output and the published span's `style` attribute after the sanitizer round trip. The pre-existing toolbar-button test was re-scoped to `[data-testid="editor"] [data-addon-slot="actions.attach"]` to resolve a new strict-mode collision from the second editor, without weakening any of the three original assertions. `npm run e2e -- rte-actions` passed 6/6 against a real pristine-app install (fresh `ng build`/serve + Playwright); `git show 4ac59f25 --stat` confirmed only `e2e/**` files changed. SonarQube server scan (§4 DONE gate) could not be run in this environment — Docker unavailable and no `SONAR_TOKEN` — flagged as outstanding, not silently skipped, consistent with every prior task in this plan. |
