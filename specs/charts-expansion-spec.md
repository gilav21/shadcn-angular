...
| 2026-06-29 | Phase 1 — line-chart, area-chart, combo-chart (+ `bandAreaPath`) | **95** (family gate) | TDD: line 13, area 8, combo 7. Crosshair + interactive legend + monotone/step curves; area stacked (absolute/percent); combo dual-axis Pareto. Demo (10 locales) + stories + e2e (3/3 pass). Caught & fixed AOT-only `private area()` template bug via e2e. |
| 2026-06-29 | Phase 2 — scatter-chart, bubble-chart | **95** (family gate) | TDD: scatter 7, bubble 6. 2-D nearest-point hover, sqrt sizeScale for bubbles. New types XYDataPoint/XYZDataPoint/XYSeries/XYZSeries (additive). Demo + stories + e2e (2/2 pass). |
| 2026-06-29 | Phase 3 — gauge-chart, radar-chart, bullet-chart (+ `chart-polar`) | **95** (family gate) | TDD: gauge 7, radar 7, bullet 7, chart-polar 5. Threshold zones, spider polygons + interactive legend, KPI bar with target. New type GaugeThreshold. Demo + stories + e2e (3/3 pass after fixture-fix). |
| 2026-06-29 | Phase 4 — heatmap, calendar-heatmap | **95** (family gate) | TDD: heatmap 6, calendar 7. Sequential HSL color scale, GitHub-style week×weekday calendar. New types HeatmapCell/CalendarDay. Demo + stories + e2e (2/2 pass). |
| 2026-06-29 | Phase 5 — funnel-chart, waterfall-chart | **95** (family gate) | TDD: funnel 7, waterfall 7. Conversion funnel (vs-first/vs-previous %), running-total bridge with connectors + total columns. New types WaterfallBar/WaterfallPointType. Demo + stories + e2e (2/2 pass). |
| 2026-06-29 | Phases 1–5 family review gate | **95** | Independent reviewer (fresh context) over all 14 charts + chart-polar + bandAreaPath. Additive invariant verified (chart.types.ts appends only; package.json/lock empty diff; existing charts/chart.utils.ts untouched). Math confirmed correct (waterfall totals, gauge clamp+thresholds, Pareto cumulative, funnel %, bubble sqrt sizing, calendar Sunday-origin weeks). SonarQube-clean, AOT-safe, behavioral tests. Fixed post-review: waterfall connectors now span the inter-bar gap only (were over-drawing behind bars). |
...
| 2026-07-01 | RTL hover-position fix (heatmap, calendar-heatmap) | n/a | User-reported: no hover tooltip in RTL locales (Hebrew). Verified in-browser: the block SVG right-aligned under RTL while tooltip coordinates stayed relative to the container's physical-left edge, offsetting the tooltip ~1240px from the cursor. Fixed by sizing the container `w-fit` with a physical `mr-auto` and sizing the SVG to its intrinsic content width instead of the full container. Also fixed heatmap's elongated (non-square) cells and left-aligned the calendar-heatmap grid with its legend per user report. |
| 2026-07-02 | Merge master + lint reconciliation | n/a | Merged `origin/master` (CLI 3-way merge/apply engine, data-table context-menu addon, SonarJS rule widening) into this branch — clean merge, no conflicts. `sync-registry --fix` reported no drift. Master's widened SonarJS rule set flagged 21 pre-existing findings in this branch's chart specs (`sonarjs/prefer-specific-assertions` generic `.length` checks, `@angular-eslint/no-output-native` on chart-legend's `toggle` output, one dead-store var) — fixed all: converted assertions to `toHaveLength()`, renamed `toggle` → `itemToggle`, removed the unused var. Post-merge full verification: `npm run lint` clean, `typecheck` + `typecheck:templates` clean, full unit suite 5160/5160 passing (286 files), CLI suite 546/546 passing, demo AOT-builds across all 10 locales, and the e2e impact-selected 13 chart specs + `data-table` all pass (14/14). No dependency drift (`package.json`/`package-lock.json` untouched by this branch). |
