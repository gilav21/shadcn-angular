...
| 8 | e2e: `ComponentSpec.addArgs?` + `run.ts` splice; `EXPLICIT_SPECS` entry `{ names: ['rich-text-editor'], addArgs: ['--preset', 'writing'], label: 'rte-preset-writing', harnessFolder: 'rte-links' }`; `npm run e2e -- rte-preset-writing` green | UC-19 | ✅ Done | 2026-09-04 05:36 | — | Registered exactly as specced; verified the loader resolves it (`names`, `addArgs`, harness `rte-links`, 187 specs, no duplicate labels). ⚠️ Spec correction: the worktree had an EMPTY `node_modules`, so both `npm run e2e` and the coverage leg of `sonar:gate` failed on a missing `node_modules/vitest/vitest.mjs` until a local `npm install` was run — worth noting for any future worktree-isolated agent. |
