...
| 9 | Gates: `npm run test:cli`, `npm run coverage:cli`, `npm run lint`, `npm run docs:check`, `npm run sonar:gate`; re-verify the publish boundary against `registry/load.ts` + `core/fetch.ts` and record "**CLI publish required**" in the completion log | all | 🔴 BLOCKED (not done) — needs a maintainer decision | — | — | `test:cli` 1474 passed / 1 failed (the pre-existing `gen-file-sizes-lib` CRLF artifact, verified to fail identically with every change stashed). `lint` 0 errors. Coverage on the changed files: plan.ts 100% lines, presets.ts 100%, why-core.ts 100%, add.ts 98.71%, load.ts 98.3%. `docs:check` green except `file-sizes.json is stale` — also verified pre-existing (same CRLF artifact). e2e green (see row 8). Sonar: first `npm run sonar` attempt died with EXECUTION FAILURE — an IOException in the file-tree walk, caused by racing a concurrent `npm install` writing into `node_modules`, not by the code. Re-run clean: EXECUTION SUCCESS in 8:50, **`new_violations` = 0** and **0 open issues on the project**. But the quality gate still reported ERROR on `new_coverage = 0.0` because `npm run sonar` uploads no coverage — exactly the stale/missing-coverage case the gate exists to catch. `npm run sonar:gate` (coverage measured first) is the run that counts. ⚠️ **Repo-level finding, pre-existing on master and NOT caused by this work:** `packages/components/file-sizes.json` disagrees with the committed sources. For `accordion/accordion.component.html` the manifest says 84 bytes while `git show master:…` gives **81** — the manifest was generated on a checkout that wrote CRLF to disk, so the committed numbers are inflated by one byte per line. This is what makes `gen-file-sizes-lib.spec.ts > matches the files actually on disk` fail and `docs:check` report `file-sizes.json is stale`. It also **blocks `sonar:gate`**, because `scripts/coverage.mjs:44-49` withholds the tree fingerprint when either leg has any failing test. Deliberately NOT "fixed" here: regenerating would rewrite ~1000 manifest entries and bury a drift the maintainer should decide about, and it is outside this spec's scope. **Final `sonar:gate` state:** browser leg now fully green (510 files / 10,645 tests, exit 0) after repairing the worktree's partially-installed `typescript` and `rxjs`; CLI leg 1474 passed / **1 failed — that one test and nothing else**. So the gate is blocked by repo data, not by this branch's code, and the scan-only evidence stands: `new_violations` 0, 0 open issues project-wide, 0 on the eleven changed files. **Maintainer decision needed:** either regenerate `file-sizes.json` on a canonical checkout (`npm run docs:sizes`) or add a `.gitattributes` eol policy, then re-run `sonar:gate` to get a coverage-backed PASS. |
...
| 2 | 2026-09-04 | Tasks 3–8 — presets, what-now block, demo, e2e | 88 (re-review pending) | First review found a real regression: `selectAddons` had `--all` hoisted above `--yes`, silently changing pre-existing `add --all --yes` from installing no addons to installing every one. Fixed by restoring §D.4(b)'s order, plus two tests that guard the ORDER (the previous one omitted `--yes` and could not discriminate it) and a verbatim pin of the §D.3 stale-manifest warning. Re-review requested; no reply at time of writing. **Publish: REQUIRED** — CLI logic + `registry/load.ts` validator + `ComponentDefinition` shape all changed. |
| 3 | 2026-09-04 | Task 9 — gates | — | 🔴 BLOCKED. Scan clean (`new_violations` 0; 0 open issues; 0 on the 11 changed files) but the quality gate fails `new_coverage = 0.0`, because `file-sizes.json` is stale **on master** (manifest 84 bytes vs an 81-byte blob, a CRLF-generation artifact) and `coverage.mjs` withholds the fingerprint on any failing test. Not self-fixed: regenerating rewrites ~1000 entries and hides a real drift. |
