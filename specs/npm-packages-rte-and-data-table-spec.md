...
| ⚠️ C-16 *(added during execution, 2026-09-04)* | T-6 as written: "the **entry** must contain `import('./` and must **not** contain `function parsePdfReadable`, and a chunk must." | Measured against the real `ng build rte-package` output: ng-packagr emits the entry FESM (`gilav21-shadcn-angular-rte.mjs`, **3.5 kB**) as a pure **re-export barrel** — one `export { … } from './…-B8BHu-C6.mjs'` line, zero declarations and **zero** dynamic imports. All 3 `import('./…')` calls live in the 1.7 MB main chunk the entry statically imports. So the spec's literal assertion is unimplementable: "the entry contains `import('./`" is false on a correct build, and "the entry does not contain `parsePdfReadable`" is trivially true no matter what ng-packagr does — a parser inlined into the **main chunk** (the actual C-14 regression) would satisfy both and pass. | `assertLazyChunks` asserts the **invariant behind** T-6 instead of its literal text: walk the eager graph (entry + everything reachable by **static** import), require ≥ 1 bundle outside it, require ≥ 1 surviving `import()` from the eager graph, and require `function parsePdfReadable` to appear **only outside** the eager graph. This is strictly stronger — it fails the main-chunk-inlining case the literal wording misses. Verified by sabotage: parser inlined into the eager main chunk → throws; all bundles statically reachable → throws; single inlined bundle → throws; parser only in a lazy chunk → passes; the real build output → passes. |

| ⚠️ C-17 *(added during execution, 2026-09-04 — SPEC CHANGE from the lead)* | C-6 pinned `peerDependencies` to `^21.0.0`; C-8 and §D option set C concluded that "a 21-compiled partial-Ivy library cannot be linked by a 20 consumer" and therefore that a second Angular-21 fixture was needed to test the packages at all. | **The premise is false, and it was measured.** (1) The compiled output's own compatibility floor is the `minVersion` stamped into each partial-Ivy declaration; across BOTH packages the distinct values are `12.0.0, 14.0.0, 17.0.0, 17.1.0, 17.2.0` — **maximum 17.2.0**, nowhere near 20. Partial-Ivy is forward-compatible: an app on the same or a NEWER major links it, so building at 21 does not exclude 20 — only the declared peer range did. (2) Proven end-to-end, not inferred: a real Angular **20.3.30** app (copy of `e2e/fixture-app`, 495 packages) installed the Angular-21-toolchain-built tarball and ran `ng build --configuration production` to **"Application bundle generation complete"**, rendering `<ui-rich-text-editor uiRteFull [(ngModel)]>`. With the `^21.0.0` range the same install failed `ERESOLVE`; with the widened range it succeeded. The lazy parser chunks also survived into the consumer bundle (`chunk-…-pdf-readable` 160.5 kB, `docx-parser` 23.95 kB, `docx-to-editor-html` 7.19 kB as **lazy** chunks). | Peer range for `@angular/{common,core,forms,platform-browser}` becomes **`>=20.0.0 <22.0.0`** (`rxjs ^7.8.0` unchanged), asserted by `assertPackedManifest` via the exported `ANGULAR_PEER_RANGE`. **No second toolchain and no downgrade is required** — the packages continue to be *built* with the workspace's Angular 21 / ng-packagr 21, because the artifact is already 20-consumable. `e2e/fixture-app-21/` is **kept** as the Angular 21 leg (it is now the only proof behind the README's "Angular 21" claim), and the package legs run against **both** fixtures. Budget note confirmed by measurement: the RTE closure bundles to **1.11 MB initial**, which fails the ng20 fixture's existing 1 MB error budget — so whichever fixture runs a production `ng build` needs the raised **2 MB / 4 MB** budgets. |

| ⚠️ C-18 *(added during execution, 2026-09-04)* | C-15 raised the budgets on the NEW fixture only, on the assumption that the packages would be tested solely there. | C-17 moved the ng20 fixture into the package legs too, so `e2e/fixture-app` now runs a production build of the RTE closure (~1.11 MB initial) and its original 500 kB warn / **1 MB error** budget fails. Raising it to 2 MB / 4 MB fixes that — but `e2e/fixture-app/angular.json` is shared with the pre-existing CLI specs that also build for production there (`add-all-smoke`, `prod-build`, `migrate-build`), so their only size guard was raised along with it. | Both fixtures use **2 MB / 4 MB**. The consequence is recorded here and in `e2e/README.md` rather than left silent: a copy-model size regression under 4 MB no longer trips those three specs. Scoping the budget per spec would need a second Angular configuration in the fixture; §B.4 puts bundle-size budgets out of scope, so this is a **knowing** gap, not an oversight. |
...
|---|------|--------|--------|-----------|-------|---------------|
| 1 | Write the failing tests: `stage-package-lib.spec.ts` (T-1…T-5, T-7, T-8), `stage-package.spec.ts` (T-24), `release-package-lib.spec.ts` (T-10…T-13), `release-package.spec.ts` (T-14…T-16), `e2e/orchestrator/specs.spec.ts` (T-17), `impact.spec.ts` additions (T-18), the three Playwright specs (T-19…T-21), and the repo-excludes drift test (T-9). Run `npm run test:cli -- --run` and confirm every new test fails for the right reason (missing module / wrong output), then sabotage-check each one's contract per memory rule. | all UC | ✅ Done | 2026-09-04 04:30 | 93 | Writing all 24 tests first paid off immediately: T-4 (the import-exactness audit) independently caught a staging bug during sabotage. Two tests were later found weak by review (a vacuous spec/stories filter test and an over-specified addon-order assertion) and were fixed — worth deriving sabotages from the contract, not the assertions, next time. |
| 2 | `stage-package-lib.ts` + `stage-package.ts` entry; root `stage:package` script; `workspaces` pin; repo excludes (`tsconfig.json`, `tsconfig.eslint.json`, `eslint.config.mjs`, `sonar-project.properties`, `.gitignore`). T-1…T-5, T-7, T-9, T-24 green. `npm run check:all` unchanged after staging both packages. | UC-1, UC-2, UC-3, UC-4, UC-17 | ✅ Done | 2026-09-04 04:38 | 93 | Deriving the file set from the registry rather than a hand-list made exactness testable, and the tsconfig/eslint/sonar excludes held on the first try (tsc reports zero errors from 273 staged files). The public-API scope needed one correction: roots alone exported no addons, since the 13 RTE addons are transitive deps of the `full` marker. |
| 3 | Library projects: `ng-packagr` devDep (review lock diff), `angular.json` projects, `packages/{rte,data-table}-package/{package.json,ng-package.json,tsconfig.lib.json}`; `package-build.ts` (+ `buildPackageTarball` with T-6/T-23 assertions) and `build:package` script. `npm run build:package -- rte` and `-- data-table` both succeed; inspect `dist/*/fesm2022` for the lazy chunks and record their names in the retrospective. | UC-5, UC-6, UC-7 | ✅ Done | 2026-09-04 04:44 | 93 | T-6 passed on the real build — ng-packagr kept all 4 parser chunks lazy (~680 kB off the entry). The laziness gate itself was initially vacuous (it checked the entry FILE, but ng-packagr emits a thin re-export barrel) and had to be rewritten to walk the eager static-import graph; the lockfile also needed surgical merging because Windows npm prunes 45 cross-platform binaries. |
| 4 | READMEs for both packages rendered around `consumerCssSnippet` (T-8 green); `docs/local-gates.md`, `e2e/README.md`, `.claude/CLAUDE.md` paragraph. | UC-11 | ✅ Done | 2026-09-04 15:05 | 93 | Both READMEs render around the exact `consumerCssSnippet` string the e2e legs write into the fixture, so the documented contract and the tested one cannot drift (T-8). Adding README.md to the tarball gate first — before writing the files — made the gap visible immediately. |
| 5 | `e2e/fixture-app-21/` + orchestrator support: `paths.ts`, `reset-app.ts` (parameterised; `e2e:reset` resets both), `worker.ts` (ng21 worker), `specs.ts` fields + validation + three entries, `run.ts` package branch (tarball install, non-CLI Tailwind setup, prod `ng build`), `impact.ts` package rules. T-17, T-18 green. | UC-15, UC-16 | ✅ Done | 2026-09-04 15:05 | — | The ng21 fixture and both orchestrator halves landed; the one real bug was a stale `buildTarget` project name that `ng build` tolerates (it resolves the sole project) but `ng serve` does not — it surfaced as a bare exit 127 and took two attempts to fix, the first because a regex lookahead silently matched nothing. Verify a rename by COUNT, not by eyeballing the diff. |
| 6 | `e2e/harness/pkg-rte/` and `e2e/harness/pkg-data-table/` demo + spec; `npm run e2e -- pkg-rte pkg-data-table` green (T-19, T-20, T-22). | UC-8, UC-9 | ✅ Done | 2026-09-04 15:05 | — | pkg-rte 3/3 and pkg-data-table 5/5 on both Angular majors. The Tailwind assertion had to be rewritten to target `ui-rich-text-toolbar` (host class `block`, vs a custom element's default `inline`) — the first selector matched nothing and would have been a silently vacuous test. |
| 7 | `e2e/harness/pkg-mixed/`; `npm run e2e -- pkg-mixed` green (T-21). | UC-10 | ✅ Done | 2026-09-04 15:05 | — | pkg-mixed 4/4: a CLI-copied button and the compiled editor coexist, both DI graphs intact. `data-slot="button"` sits on the inner element, not the `<ui-button>` host — caught only because the assertion actually ran. |
| 8 | `release-package-lib.ts` + `release-package.ts`; parameterise `releaseCommitArgv` / `prependRelease` in `release-cli-lib.ts` with CLI defaults (`release-cli.spec.ts` unchanged and green); root `release:package` script. T-10…T-16 green; `npm run release:package -- rte patch --dry-run` on the real repo prints the expected rehearsal. | UC-12, UC-13, UC-14 | ✅ Done | 2026-09-04 16:20 | 93 | Review found a real first-release bug: the revert was a blanket `git checkout` over a CHANGELOG.md that does not exist yet, so a preflight failure died mid-revert and left exactly the dirty tree the revert prevents. Now decided per path by whether git tracks it, and covered by a sabotage-verified subprocess test. |
| 9 | Final gates: `npm run preflight`, `npm run e2e -- pkg-rte pkg-data-table pkg-mixed`, `npm run e2e:impact -- --base origin/master` (shows the three labels), `npm run sonar:gate` clean on all changed files; update `project_pending-releases.md`; fill the completion log. | all UC | ✅ Done | 2026-09-04 16:20 | 93 | Sonar reports 0 unresolved issues project-wide and new_violations 0/0; the CLI coverage ratchet holds at 75.34/70.76/79.27/76.23. All 5 package e2e labels pass on both Angular majors. The one preflight failure (gen-file-sizes, 3-byte CRLF) is a worktree checkout artifact — this branch changes zero files under packages/components/. |
...
|---|---|---|---|
| 2026-09-04 | Tasks 1–3 (+ the READMEs of Task 4) | 93 | Three review rounds (88 → 90 → 93), each finding a real defect the previous sabotage had missed. Round 1: `assertLazyChunks` was substantially vacuous — ng-packagr emits the entry FESM as a 3.5 kB pure re-export barrel, so "the marker is not in the entry file" guarded an impossible condition and a parser inlined into the statically-imported 1.7 MB main chunk passed silently; rewritten to walk the eager static-import graph. Round 2: UC-5/UC-7 require `README.md` in the tarball but `REQUIRED_ENTRIES` omitted it, so that slice of T-23 could never fire and the real tarball shipped no README. Round 3 verified both fixes independently (reproducing the sabotage matrix and confirming the T-8 drift test genuinely couples README text to `consumerCssSnippet`), plus the closure counts, the UC-17 exclusion via `tsc --listFiles`, and the lockfile graft (0 entries lost, 11 gained). Remaining nit, accepted: `assertTarballContents` / `assertPackedManifest` are pure functions with no unit tests, verified only through a multi-minute build. |
| 2026-09-04 | Tasks 4–9 | 93 | Two review rounds (89 → 93). Round 1 reproduced a real defect: the preflight revert used a blanket `git checkout --` over `CHANGELOG.md`, which is untracked before a package's first release, so git exited 1 mid-revert and left the bumped `package.json` plus an untracked changelog — the exact dirty tree the revert exists to prevent, on an untested path. It also found that `pkg-mixed` never registered the package's `@source` (so it could not catch a mixed-mode styling regression) and that `lib/utils.ts` scheduled zero specs despite being staged into both packages. All fixed and sabotage-verified. Round 2 verified each fix independently and flagged two documentation-accuracy defects — an unrecorded consequence of raising the shared ng20 production budget, and a comment claiming a fall-through that cannot happen — both now corrected (C-18, and `lib/utils.ts` returns `all`). |
