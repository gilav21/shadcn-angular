...
| --- | --- | --- |
| `ComponentPoolService` | `lib/component-pool.service.ts` | Recycles `ComponentRef`s with a 200-instance cap. **CORRECTION (2026-08-20, canvas-engine Task 1):** it is *not* reusable as-is for the canvas. Items are projected via a `TemplateRef`, which yields `EmbeddedViewRef`s, not `ComponentRef`s — the two have no common recycling API. `infinite-canvas` therefore ships its own local embedded-view recycler (`ViewContainerRef.detach()` / `insert()`) exposing the same `createCount` / `recycleCount` counters. `ComponentPoolService` is deliberately left untouched (other components depend on it). It remains the right prior art for the *shape* of a recycler, not a drop-in dependency. |
...
- **Nodes are DOM**, absolutely positioned in world coordinates inside a single
  ...
  exist; they are created and recycled through a local embedded-view recycler
  modelled on `ComponentPoolService` (see the correction in §1). DOM
