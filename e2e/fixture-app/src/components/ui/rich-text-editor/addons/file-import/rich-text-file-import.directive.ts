...
import { createLocaleBindings, type LocaleInput } from '@/components/lib/i18n';
import {
...
export class RichTextFileImportDirective {
    ...
    private async importDocx(file: File): Promise<void> {
        ...
        const { parseDocx } = await import('@/components/lib/parsers/docx-parser');
        const { renderDocxForEditor } = await import('@/components/lib/parsers/docx-to-editor-html');
    ...
    private async importPdf(file: File): Promise<void> {
        ...
        const { parsePdfReadable } = await import('@/components/lib/parsers/pdf-readable/pdf-readable');
