...
/** The only files a release commit may contain. */
export const RELEASE_PATHS = ['packages/cli/package.json', 'packages/cli/CHANGELOG.md'] as const;

/**
 * The `git add` / `git commit` argv of the release commit — pathspec-scoped, so
 * whatever else happens to be staged (very reachable under `--allow-dirty`) is
 ...
 * it needs no publish.
 *
 * FAILS SAFE. The stripper is line-wise (the literal is machine-generated by
 * `sync-registry --fix`, one entry per line, closed by a `});` at column 0) and
 * deliberately NOT a parser: registry `description` fields carry free text —
 * `new:component --description "a smiley :("` is enough to unbalance a
 * character-wise paren scan. If the literal's opening or closing line cannot be
 * located, the WHOLE source is returned. Two revisions then compare unequal
 * whenever anything in the file changed, i.e. the verdict errs towards
 * "publish REQUIRED". A parse failure must never be able to say "no publish
 * needed" — that ships a CLI fix to nobody.
 */
export function stripRegistryData(source: string): string {
    const lines = source.split('\n');
    const start = lines.findIndex((line) => line.includes('registry = defineRegistry('));
