...
// ---------------------------------------------------------------------------
/** `export { A, B };` block of a rendered composite barrel. */
const EXPORT_BLOCK_RE = /export \{([^}]*)\};/;
/** `export const NAME = [ … ] as const;` block of a rendered composite barrel. */
const arrayBlockRe = (name: string): RegExp =>
    new RegExp(String.raw`export const ${name} = \[([^\]]*)\] as const;`);

/** The class names listed inside one of those blocks, in source order. */
function namesIn(re: RegExp, source: string): string[] {
    const found = re.exec(source);
    if (!found) return [];
    return found[1].split(',').map(part => part.trim()).filter(Boolean);
}
...
describe('analyzeComposites', () => {
    ...
    // T-1 — the NG3004 contract: a directive reachable through `imports: [X_FULL]`
    // must ALSO be a named export of the barrel that declares the array, or a
    // consumer's AOT build fails with "not exported from …/full". The header and
    // import assertions above do not guard that; this does.
    it('named-re-exports every sibling class next to the array', () => {
        const out = renderCompositeBarrel('X_FULL', [
            { short: 'aaa', className: 'AaaDir', sourceFile: 'ed/addons/aaa/aaa.directive.ts' },
            { short: 'bbb', className: 'BbbDir', sourceFile: 'ed/addons/bbb/bbb.directive.ts' },
        ]);
        expect(namesIn(EXPORT_BLOCK_RE, out)).toEqual(['AaaDir', 'BbbDir']);
        // Every array member is also a named export — that is the whole contract.
        expect(namesIn(arrayBlockRe('X_FULL'), out)).toEqual(namesIn(EXPORT_BLOCK_RE, out));
    });

    it('renders an empty export block and array for a composite with zero siblings', () => {
        const out = renderCompositeBarrel('X_FULL', []);
        expect(namesIn(EXPORT_BLOCK_RE, out)).toEqual([]);
        expect(namesIn(arrayBlockRe('X_FULL'), out)).toEqual([]);
    });

    // T-2 — a fourteenth addon must join BOTH lists with no hand edit.
    it('adds a new sibling to both the export block and the array', () => {
        compTouch('ui/ed/addons/ccc/ccc.directive.ts', directiveSource('EdCccDirective', 'edCcc'));
        const entries = [
            ...compositeEntries(),
            entry({
                name: 'ed/ccc', type: 'addon', parent: 'ed',
                files: ['ed/addons/ccc/index.ts', 'ed/addons/ccc/ccc.directive.ts'],
                attachImport: "EdCccDirective from './ui/ed/addons/ccc'", attachSelector: 'edFull',
            }),
        ];
        const barrelEdit = analyzeComposites(entries, compRoots).edits.find(e => e.kind === 'barrel');
        expect(barrelEdit).toBeDefined();
        const content = (barrelEdit as { content: string }).content;
        expect(content).toContain("import { EdCccDirective } from '../ccc';");
        expect(namesIn(EXPORT_BLOCK_RE, content)).toContain('EdCccDirective');
        expect(namesIn(arrayBlockRe('ED_FULL'), content)).toContain('EdCccDirective');
    });
});

// ---------------------------------------------------------------------------
// Portable-test collection & validation (--include-tests)
// ---------------------------------------------------------------------------

describe('lintPortableSpec', () => {
    const clean = "import { describe, it, vi } from 'vitest';\nvi.fn();\n";

    it('passes a spec with an explicit vitest import and no unshimmable APIs', () => {
        expect(lintPortableSpec('x.spec.ts', clean)).toEqual([]);
    });

    it('flags a missing vitest import (globals reliance)', () => {
        const problems = lintPortableSpec('x.spec.ts', 'describe("x", () => {});');
        expect(problems.some(p => p.includes("from 'vitest'"))).toBe(true);
    });

    it('flags vi.mock as unshimmable', () => {
        const src = clean + "vi.mock('./dep');\n";
        expect(lintPortableSpec('x.spec.ts', src).some(p => p.includes('vi.mock'))).toBe(true);
    });

    it('flags vi.hoisted as unshimmable', () => {
        const src = clean + 'const h = vi.hoisted(() => ({}));\n';
        expect(lintPortableSpec('x.spec.ts', src).some(p => p.includes('vi.hoisted'))).toBe(true);
    });

    it('flags vi.useFakeTimers with an options object', () => {
        const src = clean + 'vi.useFakeTimers({ shouldAdvanceTime: true });\n';
        expect(lintPortableSpec('x.spec.ts', src).some(p => p.includes('useFakeTimers'))).toBe(true);
    });

    it('accepts a bare vi.useFakeTimers()', () => {
        expect(lintPortableSpec('x.spec.ts', clean + 'vi.useFakeTimers();\n')).toEqual([]);
    });
});

describe('dependencyClosure', () => {
    it('walks transitive runtime dependencies', () => {
        const deps = new Map<string, readonly string[]>([
            ['a', ['b']], ['b', ['c']], ['c', []],
        ]);
        expect([...dependencyClosure('a', deps)].sort((a, b) => a.localeCompare(b))).toEqual(['b', 'c']);
    });

    it('tolerates cycles', () => {
        const deps = new Map<string, readonly string[]>([['a', ['b']], ['b', ['a']]]);
        expect([...dependencyClosure('a', deps)].sort((a, b) => a.localeCompare(b))).toEqual(['a', 'b']);
    });
});

describe('updateEntryArray / removeEntryArray', () => {
    const source = "  'button': {\n    name: 'button',\n    files: ['button/button.component.ts'],\n  },\n";

    it('inserts a testFiles array after files when absent', () => {
        const out = updateEntryArray(source, 'button', 'testFiles', "'button/button.component.spec.ts'", ['dependencies', 'files']);
        expect(out).toContain("testFiles: ['button/button.component.spec.ts']");
    });

    it('round-trips insert then remove back to the original', () => {
        const withArr = updateEntryArray(source, 'button', 'testFiles', "'x.spec.ts'", ['files']);
        expect(removeEntryArray(withArr, 'button', 'testFiles')).toBe(source);
    });

    it('does not confuse testFiles with files (camelCase marker)', () => {
        const withArr = updateEntryArray(source, 'button', 'testFiles', "'x.spec.ts'", ['files']);
        expect(withArr).toContain("files: ['button/button.component.ts']");
        expect(withArr).toContain("testFiles: ['x.spec.ts']");
    });
});

describe('collectPortableSpecs & analyzePortableTests', () => {
    let ptRoot: string;
    let ptRoots: SyncRoots;

    function write(rel: string, content: string): void {
        const full = path.join(ptRoot, 'components', rel);
        mkdirSync(path.dirname(full), { recursive: true });
        writeFileSync(full, content);
    }

    beforeAll(() => {
        ptRoot = mkdtempSync(path.join(tmpdir(), 'portable-tests-'));
        ptRoots = { componentsRoot: path.join(ptRoot, 'components'), blocksRoot: path.join(ptRoot, 'blocks') };
        write('ui/badge/badge.component.ts', "export class Badge {}\n");
    ...
    afterAll(() => rmSync(ptRoot, { recursive: true, force: true }));
    ...
    function entryFor(name: string, files: string[], testFiles: string[] = []): RegistryEntry {
        return { name, files, libFiles: [], dependencies: [], isBlock: false, testFiles, testDependencies: [] };
    }

    function updateFor(name: string, files: string[]): ComponentUpdate {
        return { name, files, libFiles: [], dependencies: [] };
    }

    function ctxFor(entries: RegistryEntry[]): BoundaryContext {
        const map = buildBoundaryMap(entries);
        return { entryFileToComponent: map, dirOwners: buildDirOwners(map) };
    }

    it('emits testFiles only for verified components', () => {
        const entries = [entryFor('badge', ['badge/badge.component.ts']), entryFor('select', ['select/select.component.ts'])];
        const updates = [updateFor('badge', ['badge/badge.component.ts']), updateFor('select', ['select/select.component.ts'])];
        const config: PortableTestsConfig = { verified: ['badge'] };

        const result = analyzePortableTests(entries, updates, config, ctxFor(entries), ptRoots);
        expect(result.errors).toEqual([]);
        expect(result.updates.get('badge')?.testFiles).toEqual(['badge/badge.component.spec.ts']);
        expect(result.updates.has('select')).toBe(false);
    });

    it('errors when a verified component has no portable specs on disk', () => {
        const entries = [entryFor('empty', ['empty/empty.component.ts'])];
        const updates = [updateFor('empty', ['empty/empty.component.ts'])];
        const result = analyzePortableTests(entries, updates, { verified: ['empty'] }, ctxFor(entries), ptRoots);
        expect(result.errors.some(e => e.includes('no portable specs'))).toBe(true);
    });

    it('revokes testFiles for a component dropped from the verified list', () => {
        const entries = [entryFor('badge', ['badge/badge.component.ts'], ['badge/badge.component.spec.ts'])];
        const updates = [updateFor('badge', ['badge/badge.component.ts'])];
