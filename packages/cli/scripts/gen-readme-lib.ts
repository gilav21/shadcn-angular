...
export function renderFacts(facts: ReadmeFacts): string {
    ...
    return [
        ...
        '',
        'Two of the largest components are *also* published as ordinary compiled ' +
        'packages for teams that would rather take a dependency than own the source: ' +
        '`@gilav21/shadcn-angular-rte` and `@gilav21/shadcn-angular-data-table`. They ' +
        'are an alternative, not a requirement — the copy model above never needs them.',
        '',
        '## Tested versions',
        '',
