...
/**
 * The demo file each component resolves to must exist. This used to also assert
 * a StackBlitz URL was built from it — a check on the URL's SHAPE that passed
 * for months while the link itself hung forever on "Cloning repo from GitHub".
 * The link is gone (see specs/stackblitz-playground-spec.md); the part of the
 * check that was about reality — the file exists — is kept.
 */
describe('every linked component resolves to a demo file that exists', () => {
    it('references a real demo source', () => {
        const linked = committed.components.filter(c => c.demoFile !== null);
    ...
    it('resolves a demo file exactly when it resolved a demo route', () => {
        for (const component of committed.components) {
            expect(component.demoFile === null).toBe(component.demoRoute === null);
        }
    });
});
...
describe('buildComponentDocs', () => {
    ...
    it('resolves the demo file', () => {
        expect(built.components[0].demoFile)
            .toContain('box-demo.component.ts');
    });

    it('leaves the demo file null when there is no demo', () => {
        const noRoutes = buildComponentDocs(FIXTURE_REGISTRY, FIXTURE_DOCS, []);
        expect(noRoutes.components[0].demoFile).toBeNull();
