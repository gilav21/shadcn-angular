...
/** Commands that install or reconcile component files. */
function registerInstallCommands(program: Command): void {
    program
        .command('init')
...
/** Commands that reconcile already-installed components with the registry. */
...
/** Read-only registry / project introspection. */
function registerInspectCommands(program: Command): void {
    program
        .command('list')
        .description('List all components and their install status')
        .option('--remote', 'Force remote fetch from GitHub registry')
        .option('-b, --branch <branch>', 'GitHub branch to read the registry from', 'master')
        .option('-r, --registry <url>', 'Custom registry base URL')
        .action(list);
...
/** Repo maintenance and the MCP server entrypoint. */
...
/** Design-token setters (density, radius, motion, locale, theme). */
function registerSettingsCommands(program: Command): void {
    program
        .command('set-density')
...
/**
 * Build the fully-configured commander program.
 *
 * Kept separate from the bin entrypoint (`index.ts`) so the program — its
 * commands and their options — can be introspected without parsing argv.
 * The CLI<->MCP parity spec relies on this.
 */
