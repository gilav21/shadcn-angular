...
export const registry = defineRegistry({
  ...
  'rich-text-editor': {
    ...
    breaking: [
      ...
      { kind: 'removal', from: 'the [customToolbarItems] input + (customToolbarAction) output on <ui-rich-text-editor> (RichTextCustomToolbarItem / RichTextEditorRef types)', to: 'a toolbar slot registered through RichTextEditorAddonHost.toolbarSlots', note: 'Custom toolbar buttons are now addons: inject(RichTextEditorAddonHost).toolbarSlots.register({ id, icon, tooltip, onClick }) from a directive on the editor element. This is the only extension path now — the removed one recorded no undo entry for its inserts. Map the ref methods: ref.insertText -> host.insertTextAtCaret, ref.insertHtml -> host.insertHtmlAtCaret, ref.getSelectedText -> host.selection().text, ref.getHtmlContent -> the editor htmlOutput(), ref.focus -> host.contentRoot.focus(). See docs/rich-text-editor.md for a worked ~40-line example.', codemod: 'none' },
    ],
    shortcutDefinitions: [
      {
