...
// `variant` lives on the projected `ui-empty-media`, not on `ui-empty` itself;
// expose it via a story-local props type so the Playground can drive it.
type EmptyStoryProps = EmptyComponent & { mediaVariant: 'default' | 'icon' };

const meta: Meta<EmptyStoryProps> = {
    title: 'UI/Empty',
    ...
    argTypes: {
        class: { control: 'text', description: 'Extra classes merged onto the ui-empty host.' },
        mediaVariant: {
            control: 'select',
            options: ['default', 'icon'],
            description: 'Visual style of the projected ui-empty-media icon container.',
        },
    },
    args: {
        class: '',
...
export default meta;
type Story = StoryObj<EmptyStoryProps>;

const TEMPLATE = `
    <ui-empty [class]="class">
        <ui-empty-header>
...
export const CustomClass: Story = {
    args: { class: 'bg-muted/50 min-h-[300px]' },
    render: (args) => ({
        props: args,
        template: `
            <ui-empty [class]="class">
