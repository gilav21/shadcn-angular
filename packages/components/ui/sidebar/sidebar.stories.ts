...
/**
 * Test-scaffolding helper: forces the shared `SidebarService` (provided by
 * the nearest `ui-sidebar-provider` ancestor) into the collapsed state so the
 * `collapseMode` stories render collapsed without requiring a click on the
 * trigger. Not a shipped component.
 */
@Component({
  selector: 'story-force-collapsed',
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `<ng-content />`,
  host: { class: 'contents' },
})
class ForceCollapsedDemoComponent {
...
const meta: Meta<SidebarComponent> = {
  ...
  argTypes: {
    ...
    variant: {
      ...
      description: 'Structural style variant, reserved as a styling hook for consumers.',
    },
    collapsible: {
      control: 'boolean',
      description: 'Whether consumers may wire a collapse/expand toggle for this sidebar.',
    },
    collapseMode: {
      control: 'select',
      options: ['icon', 'hidden'],
      description: 'Width the sidebar collapses to once SidebarService reports collapsed: a narrow icon rail, or fully hidden.',
    },
    class: { control: 'text', description: 'Extra classes merged onto the host aside element.' },
  },
  args: {
    side: 'left',
    variant: 'sidebar',
...
const COLLAPSED_TEMPLATE = `
  <ui-sidebar-provider>
    <story-force-collapsed>
      ${SIDEBAR_TAG}
    </story-force-collapsed>
    ${SIDEBAR_INSET}
  </ui-sidebar-provider>`;
...
export const CollapseModeIcon: Story = {
  args: { collapseMode: 'icon' },
  render: (args) => ({
    props: args,
    template: COLLAPSED_TEMPLATE,
  }),
};

export const CollapseModeHidden: Story = {
  args: { collapseMode: 'hidden' },
  render: (args) => ({
    props: args,
    template: COLLAPSED_TEMPLATE,
  }),
