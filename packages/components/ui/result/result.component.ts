 ...
 * native `<output>` — which carries the `status` role implicitly — combined
 * with `aria-live="polite"`, for *every* status, error included. The native
 * element is used in preference to spelling the ARIA role out on a plain
 * container: assistive-tech support for it is broader, and Sonar's Web:S6819
 * asks for exactly this. Note that `classes()` applies `flex`, so the
 * element's default inline display never takes effect.
 *
 * An assertive region cuts a screen reader off mid-sentence, which a result is
 * not worth; and nothing here calls `.focus()`, so a panel appearing beside a
 * form cannot pull the caret out of it.
 *
 * ```html
