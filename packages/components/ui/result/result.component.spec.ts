 ...
 *    the user is doing. a native `<output>` (implicit `role="status"`) + `aria-live="polite"` announces it;
...
describe('ResultComponent', () => {
    ...
    describe('T-9 announces status via role/aria-live without moving focus', () => {
        /**
         * The panel is a native `<output>`, which carries `role="status"`
         * implicitly — so assert the ELEMENT, not an explicit role attribute.
         * Asserting `getAttribute('role') === 'status'` would fail here while
         * the semantics are in fact correct, and would push the component back
         * to `<div role="status">`, which Web:S6819 flags precisely because
         * the native element has broader assistive-tech support.
         */
