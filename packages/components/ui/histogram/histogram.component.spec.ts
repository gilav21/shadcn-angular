...
describe('HistogramComponent', () => {
    ...
    describe('T-18 resize', () => {
        ...
        it('re-lays out the bars when the measured width changes', async () => {
            await createFixture();
            setMeasuredWidth(400);
            const narrow = Math.max(...component.bars().map(m => m.x + m.width));

            setMeasuredWidth(900);
            const wide = Math.max(...component.bars().map(m => m.x + m.width));
