...
const meta: Meta<DockComponent> = {
    ...
    argTypes: {
        magnification: {
            ...
            description: 'The maximum width (px) an item grows to under the cursor.',
        },
        distance: {
            control: { type: 'range', min: 0, max: 300, step: 10 },
            description: 'The cursor distance (px) over which the magnification effect falls off.',
        },
        position: {
            control: 'select',
            options: ['bottom', 'top', 'left', 'right'],
            description: 'Where the dock sits and how items stack (row vs column).',
        },
        items: {
            control: 'object',
            description: 'Data-driven items (label/icon/class/active/href/onClick) rendered when no content is projected.',
        },
        class: {
            control: 'text',
            description: 'Additional CSS classes merged onto the dock container.',
        },
    },
    args: {
        magnification: 60,
...
export const LeftPosition: Story = {
    ...
    render: (args) => ({
        props: args,
        template: `
            <div class="h-[400px] w-full flex items-center justify-start bg-gray-100 relative overflow-hidden">
                <ui-dock [magnification]="magnification" [distance]="distance" [position]="position" [class]="class">
                    ${ICON_ITEMS}
                </ui-dock>
            </div>`,
...
export const RightPosition: Story = {
    ...
    render: (args) => ({
        props: args,
        template: `
            <div class="h-[400px] w-full flex items-center justify-end bg-gray-100 relative overflow-hidden">
                <ui-dock [magnification]="magnification" [distance]="distance" [position]="position" [class]="class">
                    ${ICON_ITEMS}
                </ui-dock>
            </div>`,
...
const SIMPLE_ITEMS: DockItemData[] = [
    { label: 'Home', icon: '🏠' },
    { label: 'Profile', icon: '👤' },
    ...
    render: (args) => ({
        ...
        template: `
            <div class="h-[300px] w-full flex items-center justify-center bg-gray-100 relative">
                <ui-dock [items]="items" [magnification]="magnification" [distance]="distance" [position]="position" [class]="class" />
            </div>`,
    }),
};
